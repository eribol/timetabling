...
impl From<TextMatrix> for [f32; 6] {
    fn from(val: TextMatrix) -> Self {
        ...
        match val {
            Translate(x, y) => {
                // 1 0 0 1 x y cm
                [1.0, 0.0, 0.0, 1.0, x.0 as f32, y.0 as f32]
            }
            Rotate(rot) => {
            ...
            TranslateRotate(x, y, rot) => {
                let rad = (360.0 - rot).to_radians();
                [rad.cos(), -rad.sin(), rad.sin(), rad.cos(), x.0 as f32, y.0 as f32] /* cos sin -sin cos x y cm */
            }
        }
    }
}
...
impl From<CurTransMat> for [f32; 6] {
    fn from(val: CurTransMat) -> Self {
        ...
        match val {
            ...
            TranslateRotate(x, y, rot) => {
                let rad = (360.0 - rot).to_radians();
                [rad.cos() as f32, -rad.sin() as f32, rad.sin() as f32, rad.cos() as f32, x.0 as f32, y.0 as f32] /* cos sin -sin cos x y cm */
            }
            Rotate(rot) => {
                // cos sin -sin cos 0 0 cm
                let rad = (360.0 - rot).to_radians();
                [rad.cos() as f32, -rad.sin() as f32, rad.sin() as f32, rad.cos() as f32, 0.0, 0.0]
            }
            Raw(r) => r,
            Scale(x, y) => {
                // x 0 0 y 0 0 cm
